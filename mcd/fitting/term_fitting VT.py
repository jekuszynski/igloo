...
if __name__ == '__main__':
    ...
    ax1.fill_between(x, list(mcd_comps.values())[2], color=colors[-3], lw=1.5, ls='--', label='IB-II', alpha=0.25)
    ...
    ax2.fill_between(abs_x, list(abs_comps.values())[2], color=colors[-3], lw=1.5, ls='--', label='IB-II', alpha=0.25)
    # ax2.fill_between(abs_x, list(abs_comps.values())[3], color=colors[-4], lw=1.5, ls='--', label='UV Continuum', alpha=0.2)
    ax2.fill_between(abs_x, list(abs_comps.values())[4], color=colors[-4], lw=1.5, ls='--', label='CB', alpha=0.2)
    ...
    gs.update(hspace=0)
    plt.show()
    plt.savefig(material + '_CFS_' + peak_count + 'peak_fit_zoomed.png', format='png',dpi=400,bbox_inches='tight')
