...
def animate(i): #function for animation process
    ...
    y1 = [(np.sin(4 * np.pi * (x - 0.1 * i)))/8 for x in x if x <= 4] #default sin(x) centered at y=0
    y2 = [((np.exp(-(x - 4.0)) - 1) + np.sin(4 * np.pi * (x - 0.1 * i))/8) for x in x if x > 4 and x <= 8] #exp function to show decrease in signal
    y3 = [((-np.exp(-(x - 8.0))) + np.sin(4 * np.pi * (x - 0.1 * i))/8) for x in x if x > 8] #exp function to show recovery of signal after correction
    y_convolution = np.concatenate((y1,y2,y3))
    y_smooth = signal.savgol_filter(y_convolution,11,3)
    # line.set_data(x, y)
...
anim = FuncAnimation(fig, animate, init_func=init, frames=480, interval=20, blit=True) #animate plot
